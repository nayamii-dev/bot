    ...
    addEmitter(name: string, value: Emitter) {
        if (this.emitters[name])
            throw new CustomError(
                `[[ALREADY_REGISTERED]] the event emitter ${name} is already registered`
            );
        if (
            !(
            ...
            throw new CustomError(
                `[[INVALID_LISTENER_TYPE]] the handler ${name} has an invalid type `
            );
    ...
    loadALL(): void {
        ...
        for (const mod of this.modules.values()) {
            if (!(mod.options.handler in this.emitters)) {
                throw new CustomError(
                    `[INVALID_HANDLER] on module ${mod.options.id} the handler "{mod.options.handler}" is invalid.`
                );
            }

            this.emitters[mod.options.handler][mod.options.type](
                mod.options.event,
                (...args) => this.modules.get(mod.options.id)?.run(...args)
            );
        }
    }
}
